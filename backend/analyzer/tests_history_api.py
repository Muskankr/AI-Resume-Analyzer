...
class HistoryListTests(TestCase):
    ...
    def test_requires_authentication(self):
        response = APIClient().get("/api/v1/history/")
    ...
    def test_list_payload_omits_the_heavy_fields(self):
        make_analysis(self.user)
        row = self.client.get("/api/v1/history/").data[0]
    ...
    def test_newest_first(self):
        ...
        names = [row["file_name"] for row in self.client.get("/api/v1/history/").data]
...
class HistoryPaginationTests(TestCase):
    ...
    def test_page_param_returns_an_envelope(self):
        response = self.client.get("/api/v1/history/?page=1")
    ...
    def test_second_page_holds_the_remainder(self):
        response = self.client.get("/api/v1/history/?page=2")
    ...
    def test_pages_do_not_overlap_or_skip(self):
        first = self.client.get("/api/v1/history/?page=1&page_size=10").data["results"]
        second = self.client.get("/api/v1/history/?page=2&page_size=10").data["results"]
        third = self.client.get("/api/v1/history/?page=3&page_size=10").data["results"]
    ...
    def test_page_size_is_capped(self):
        response = self.client.get(f"/api/v1/history/?page=1&page_size={HISTORY_MAX_PAGE_SIZE * 10}")
        self.assertEqual(response.data["page_size"], HISTORY_MAX_PAGE_SIZE)

    def test_page_size_alone_is_enough_to_paginate(self):
        response = self.client.get("/api/v1/history/?page_size=5")
    ...
    def test_nonsense_params_fall_back_instead_of_erroring(self):
        for query in ("?page=abc", "?page=0", "?page=-3", "?page=1&page_size=nope"):
            with self.subTest(query=query):
                response = self.client.get(f"/api/v1/history/{query}")
                self.assertEqual(response.status_code, status.HTTP_200_OK)
                self.assertEqual(response.data["page"], 1)

    def test_page_past_the_end_is_empty_rather_than_an_error(self):
        response = self.client.get("/api/v1/history/?page=99")
    ...
    def test_next_link_carries_the_requested_page_size(self):
        response = self.client.get("/api/v1/history/?page=1&page_size=10")
...
class HistoryDetailTests(TestCase):
    ...
    def test_detail_returns_the_full_text(self):
        response = self.client.get(f"/api/v1/history/{self.analysis.pk}/")
    ...
    def test_detail_does_not_expose_another_users_analysis(self):
        ...
        response = self.client.get(f"/api/v1/history/{theirs.pk}/")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_delete_still_works_on_the_same_route(self):
        response = self.client.delete(f"/api/v1/history/{self.analysis.pk}/")
    ...
    def test_delete_does_not_touch_another_users_analysis(self):
        ...
        response = self.client.delete(f"/api/v1/history/{theirs.pk}/")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class ClearHistoryTests(TestCase):
    ...
    def test_clear_removes_only_the_callers_rows(self):
        ...
        self.client.delete("/api/v1/history/clear/")
    ...
    def test_204_response_has_no_body(self):
        ...
        response = self.client.delete("/api/v1/history/clear/")
