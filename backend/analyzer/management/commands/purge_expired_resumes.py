...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        total_deleted = 0
        self.stdout.write(
            f"Purging resume files older than {days} days (cutoff: {cutoff.isoformat()})")
        ...
        if dry_run:
            ...
            deleted_tmp = remove_old_files_in_dir(
                tmp_dir, cutoff_ts, self.stdout)
        ...
        if dry_run:
            self.stdout.write(
                f"[DRY-RUN] Would scan media resumes dir: {media_resumes_dir}")
        else:
            deleted_media = remove_old_files_in_dir(
                media_resumes_dir, cutoff_ts, self.stdout)
        ...
        try:
            ...
            self.stdout.write(
                f"Found {count_qs} Resume model instance(s) older than cutoff.")
            ...
            if not dry_run:
                for r in expired_qs:
                    try:
                        ...
                        total_deleted += 1
                        self.stdout.write(
                            f"Deleted Resume model and file for id={r.id}")
                    except Exception as e:
                        self.stdout.write(
                            f"Failed to delete Resume id={r.id}: {e}")
        except Exception as e:
            self.stdout.write(f"Failed to query Resume model: {e}")

        self.stdout.write(self.style.SUCCESS(
            f"Purge complete. Total deleted items: {total_deleted}"))
