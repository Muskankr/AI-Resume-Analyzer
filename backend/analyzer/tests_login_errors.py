...
class LoginErrorTests(TestCase):
    def setUp(self):
        ...
        self.locked_user = User.objects.create_user(
            username="lockeduser", password="password123", email="locked@example.com")
        self.locked_user.is_active = False
        self.locked_user.save()

        # Unverified user (profile exists and is_verified=False)
        self.unverified_user = User.objects.create_user(
            username="unverifieduser", password="password123", email="unverified@example.com")
    ...
    def test_nonexistent_user_returns_generic_error(self):
        resp = self.client.post(
            "/api/auth/login/", {"username": "nonexistent", "password": "password123", "captcha_token": "test-captcha-token"})
        self.assertEqual(resp.status_code, 401)
        self.assertIn(
            "No active account found with the given credentials.", resp.data["detail"])
    ...
    def test_wrong_password_returns_generic_error(self):
        resp = self.client.post("/api/auth/login/", {"username": "normaluser",
                                "password": "wrongpassword", "captcha_token": "test-captcha-token"})
        self.assertEqual(resp.status_code, 401)
        self.assertIn(
            "No active account found with the given credentials.", resp.data["detail"])
    ...
    def test_locked_user_returns_locked_error(self):
        resp = self.client.post(
            "/api/auth/login/", {"username": "lockeduser", "password": "password123", "captcha_token": "test-captcha-token"})
        self.assertEqual(resp.status_code, 401)
    ...
    def test_unverified_user_returns_unverified_error(self):
        if not hasattr(UserProfile, "is_verified"):
            self.skipTest(
                "UserProfile.is_verified does not exist yet on main — see #371")
        resp = self.client.post("/api/auth/login/", {"username": "unverifieduser",
                                "password": "password123", "captcha_token": "test-captcha-token"})
        self.assertEqual(resp.status_code, 401)
