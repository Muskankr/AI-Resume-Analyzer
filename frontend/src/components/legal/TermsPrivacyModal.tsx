...
export const TermsPrivacyModal: React.FC<LegalModalProps> = ({ isOpen, onClose, onAcceptAll }) => {
        ...
        <div className="overflow-y-auto space-y-4 pr-2 flex-1 text-xs text-slate-300">
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-1">
            ...
            <p className="text-slate-400">
              Terms Version:{' '}
              <span className="font-mono text-indigo-400">{LEGAL_TERMS_VERSION}</span>
              {' | '}
              Privacy Policy:{' '}
              <span className="font-mono text-teal-400">{LEGAL_PRIVACY_VERSION}</span>
            </p>
          </div>
