...
const ACTION_STYLES: Record<string, { color: string; icon: string }> = {
  MATCH_CALCULATED: { color: '#6366f1', icon: '📊' },
  JOB_BOOKMARKED: { color: '#f59e0b', icon: '⭐' },
  APPLICATION_SUBMITTED: { color: '#22c55e', icon: '📨' },
  RESUME_TAILORED: { color: '#06b6d4', icon: '✏️' },
  MATCH_EXPORTED: { color: '#ec4899', icon: '📤' },
};

export const JobMatchTimeline: React.FC<JobMatchTimelineProps> = ({ logs }) => {
  return (
    <div style={{ padding: '16px 0' }}>
      <h3 style={{ fontSize: 14, fontWeight: 600, color: 'rgba(255,255,255,0.8)', marginBottom: 16 }}>📋 Activity Timeline</h3>
      <div style={{ position: 'relative', paddingLeft: 24 }}>
        <div style={{
          position: 'absolute', left: 8, top: 0, bottom: 0, width: 2, background: '#2d2d3f'
        }} />
        {logs.map((log) => {
          ...
          return (
            <div key={log.logId} style={{ marginBottom: 20, position: 'relative' }}>
              ...
              <div style={{
                background: '#1e1e2e', borderRadius: 10, padding: '10px 14px',
